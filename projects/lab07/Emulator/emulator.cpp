...
instr_type parse_instr(char* tok) {
	...
	// zbb hako0Lin //
	if ( streq(tok, "orc.b") ) return ORCB;
...
int parse_instr(int line, char* ftok, instr* imem, int memoff, label_loc* labels, source* src) {
	...
	if ( pscnt > 0 ) {
		...
		switch( op ) {
			...
			// zbb hako0Lin //
			case ORCB:	// Mnemonic : orc.b rd, rs
			     if ( !o1 || !o2 || o3 || o4 ) print_syntax_error( line,  "Invalid format" );
			 	    i->a1.reg = parse_reg(o1 , line);
			 	    i->a2.reg = parse_reg(o2 , line);
			     return 1;
			case ORN:	// Mnemonic : orn rd, rs1, rs2
			     if ( !o1 || !o2 || !o3 || o4 ) print_syntax_error( line,  "Invalid format" );
			 	    i->a1.reg = parse_reg(o1 , line);
			 	    i->a2.reg = parse_reg(o2 , line);
			 	    i->a3.reg = parse_reg(o3 , line);
			     return 1;
			case REV8:	// Mnemonic : rev8 rd, rs
			    if ( !o1 || !o2 || o3 || o4 ) print_syntax_error( line,  "Invalid format" );
			 	    i->a1.reg = parse_reg(o1 , line);
			 	    i->a2.reg = parse_reg(o2 , line);
			     return 1;			
			case ROL:	// Mnemonic : rol rd, rs1, rs2
			     if ( !o1 || !o2 || !o3 || o4 ) print_syntax_error( line,  "Invalid format" );
			 	    i->a1.reg = parse_reg(o1 , line);
			 	    i->a2.reg = parse_reg(o2 , line);
			 	    i->a3.reg = parse_reg(o3 , line);
			     return 1;
			case ROR:	// Mnemonic : ror rd, rs1, rs2
			     if ( !o1 || !o2 || !o3 || o4 ) print_syntax_error( line,  "Invalid format" );
			 	    i->a1.reg = parse_reg(o1 , line);
			 	    i->a2.reg = parse_reg(o2 , line);
			 	    i->a3.reg = parse_reg(o3 , line);
			     return 1;
			case RORI:	// Mnemonic : rori rd, rs1, shamt
			     if ( !o1 || !o2 || !o3 || o4 ) print_syntax_error( line,  "Invalid format" );
			 	    i->a1.reg = parse_reg(o1 , line);
			 	    i->a2.reg = parse_reg(o2 , line);
...
void execute(uint8_t* mem, instr* imem, label_loc* labels, int label_count, bool start_immediate) {
	...
	while(!dexit) {
		...
		switch (i.op) {
            ...
            case CLZ: {
                for(int j = 31; j >= 0; j--){
                    if(rf[i.a2.reg] & (((uint32_t)1)<<j)){
                        rf[i.a1.reg] = 31 - j;
                        break;
                    }
                }
                rf[i.a1.reg] = (rf[i.a2.reg] == 0)? 32: rf[i.a1.reg];
                break;
            }
            case CPOP: {
                rf[i.a1.reg] = 0;
            ...
            case MIN: rf[i.a1.reg] = (*(int32_t*)&rf[i.a2.reg]) < (*(int32_t*)&rf[i.a3.reg])? rf[i.a2.reg]: rf[i.a3.reg]; break;
			
			// hako0Lin //
			case ORCB: 		// Bitwise OR-Combine, byte granule
				{
				uint32_t mask = 0xff; 
				for(int  shamt = 0;  shamt < 32;  shamt+=8){
					if ((rf[i.a2.reg] & (mask <<  shamt)) == 0) rf[i.a1.reg] = rf[i.a1.reg] ^ (rf[i.a1.reg] & (mask <<  shamt));
					else rf[i.a1.reg] |= (mask <<  shamt);
					}
				}
				break;
			case ORN:		// OR with inverted operand
				rf[i.a1.reg] = rf[i.a2.reg] | ~(rf[i.a3.reg]); break;
			case REV8:		// Byte-reverse register |B3|B2|B1|B0| -> |B0|B1|B2|B3|
				{ 
				uint32_t temp = 0;
				temp = ((rf[i.a2.reg] & 0xffff0000) >> 16) | ((rf[i.a2.reg] & 0x0000ffff) << 16);
				rf[i.a1.reg] = ((temp & 0xff00ff00) >>  8) | ((temp & 0x00ff00ff) <<  8);	
				}
				break;
			case ROL:		// Rotate left (Register)
				rf[i.a1.reg] = (rf[i.a2.reg] << rf[i.a3.reg]) | (rf[i.a2.reg] >> (32 - rf[i.a3.reg]));
				break;
			case ROR:		// Rotate right (Register)
				rf[i.a1.reg] = (rf[i.a2.reg] >> rf[i.a3.reg]) | (rf[i.a2.reg] << (32 - rf[i.a3.reg]));
				break;
	...
	case SEXTB:
					//sll 左移補0
					rf[i.a1.reg] = rf[i.a2.reg] << 24;
					//sra算術右移有號延伸
					rf[i.a1.reg] = (*(int32_t*)&rf[i.a1.reg]) >> 24;
					break;
			case SEXTH:
					//sll 左移補0
					rf[i.a1.reg] = rf[i.a2.reg] << 16;
					//sra(arithmetic)算術右移有號延伸
					rf[i.a1.reg] = (*(int32_t*)&rf[i.a1.reg]) >> 16;
					break;
			case ZEXTH:
					//sll 左移補0
					rf[i.a1.reg] = rf[i.a2.reg] << 16;
					//srl算術右移無號延伸
					rf[i.a1.reg] = rf[i.a1.reg] >> 16;

			case CLMUL:
					for(int j=0;j<32;j++)
					{
						if  (( rf[i.a3.reg] >> j & 1)!=0)
						{
			...
			case REM: rf[i.a1.reg] = (*(int32_t*)&rf[i.a2.reg]) % (*(int32_t*)&rf[i.a3.reg]); break;
			case REMU: rf[i.a1.reg] = (*(uint32_t*)&rf[i.a2.reg]) % (*(uint32_t*)&rf[i.a3.reg]); break;
			...
			case BEXTI: rf[i.a1.reg] = (rf[i.a2.reg] >> (i.a3.imm&0x1F)) &1; break;
      		...
      		//mt19937
			case MINU:  rf[i.a1.reg] = (rf[i.a2.reg]<rf[i.a3.reg])? rf[i.a2.reg]: rf[i.a3.reg]; break;
			case XNOR:  rf[i.a1.reg] = ~ (rf[i.a2.reg] ^ rf[i.a3.reg]); break;
			case BINV:  rf[i.a1.reg] = rf[i.a2.reg] ^ (1 << (31 & rf[i.a3.reg])); break;
			case BINVI: rf[i.a1.reg] = rf[i.a2.reg] ^ (1 << (31 & i.a3.imm)); break;
			case BSET:  rf[i.a1.reg] = rf[i.a2.reg] | (1 << (31 & rf[i.a3.reg])); break;
			case BSETI: rf[i.a1.reg] = rf[i.a2.reg] | (1 << (31 & i.a3.imm)); break;
    		//*****************//
