        ...
        switch (op) {
            ...
            case SW:
                *(uint32_t*)&(mem[addr]) = data;
                ...
                if (addr == REG_BASE_ADDRESS + REG_ENABLE && data == 1) mem[REG_BASE_ADDRESS + REG_STATUS] = 1;
                break;
...
int parse_assembler_directive(int line, char* ftok, uint8_t* mem, int memoff) {
    ...
    else if (0 == memcmp(ftok, ".zero", strlen(ftok)))
...
void execute(uint8_t* mem, instr* imem, label_loc* labels, int label_count, bool start_immediate) {
    ...
    // vector register, you can modify the length if you need
    uint64_t vrf[32];
    ...
    while (!dexit) {
        ...
        switch (i.op) {
            ...
            case SLT:
                ...
                break;
            case SLTU:
                rf[i.a1.reg] = (rf[i.a2.reg] < rf[i.a3.reg]) ? 1 : 0;
                break;
            case AND:
                rf[i.a1.reg] = rf[i.a2.reg] & rf[i.a3.reg];
                break;
			...
			*/
            ...
            case HCF:
                ...
                // * print regfile and vec_regfile >>>>>
                print_regfile(rf);
                print_vector_regfile(vrf);
                // * print regfile and vec_regfile <<<<<

                printf("\nCache read %d/%d Cache write %d/%d\n", cache_read_hits, mem_read_reqs, cache_write_hits,
                       mem_write_reqs);
