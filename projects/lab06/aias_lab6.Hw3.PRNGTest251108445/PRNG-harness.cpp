...
#endif
#include <iostream>
class PRNG_api_t: public sim_api_t<VerilatorDataWrapper*> {
    public:
    PRNG_api_t(VPRNG* _dut) {
        dut = _dut;
        main_time = 0L;
        is_exit = false;
#if VM_TRACE
    ...
    void init_sim_data() {
        ...
        sim_data.inputs.push_back(new VerilatorCData(&(dut->io_gen)));
        sim_data.outputs.push_back(new VerilatorCData(&(dut->io_ready)));
        sim_data.outputs.push_back(new VerilatorCData(&(dut->io_puzzle_0)));
        sim_data.outputs.push_back(new VerilatorCData(&(dut->io_puzzle_1)));
        sim_data.outputs.push_back(new VerilatorCData(&(dut->io_puzzle_2)));
        sim_data.outputs.push_back(new VerilatorCData(&(dut->io_puzzle_3)));
        sim_data.signals.push_back(new VerilatorCData(&(dut->reset)));
        sim_data.signal_map["PRNG.reset"] = 0;
    }
#if VM_TRACE
...
int main(int argc, char **argv, char **env) {
    ...
    VPRNG* top = new VPRNG;
    std::string vcdfile = "./aias_lab6.Hw3.PRNGTest251108445/PRNG.vcd";
